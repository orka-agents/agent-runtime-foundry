...
func acpTestConfigBytes(mode string) []byte {
	data, _ := json.Marshal(acpAgentConfiguration{
		Model: "test-model", ToolSchemaMode: mode,
		HostedTarget: acpHostedTarget{ProjectEndpoint: "https://foundry.example/api/projects/test", AgentName: "test-agent", AgentVersion: "7"},
	})
	return append(data, '\n')
}
...
func acpTestEnvironment(data []byte) map[string]string {
	return map[string]string{
		acpConfigDigestEnv: acpTestDigest(data), acpModelEnv: "test-model",
...
func TestACPConfigurationPinsExactBytesAndHostedTarget(t *testing.T) {
	data := acpTestConfigBytes(toolSchemaModeProviderStatic)
	...
	for name, mutate := range map[string]func(map[string]string){
		"wrong digest":          func(e map[string]string) { e[acpConfigDigestEnv] = "sha256:" + strings.Repeat("0", 64) },
		"uppercase digest":      func(e map[string]string) { e[acpConfigDigestEnv] = strings.ToUpper(e[acpConfigDigestEnv]) },
		"wrong model":           func(e map[string]string) { e[acpModelEnv] = "other" },
...
func TestACPMCPServerRejectsUnauthenticatedAndNonlocalTargets(t *testing.T) {
	...
	for name, data := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := acpDecode([]byte(data), &server, true); err != nil {
				return
			}
...
func TestACPConfigurationRejectsBakedToolsAndUnpinnedTargets(t *testing.T) {
	base := string(acpTestConfigBytes(toolSchemaModeRequest))
	...
	for name, raw := range cases {
		t.Run(name, func(t *testing.T) {
			data := []byte(raw)
			if _, err := decodeACPAgentConfiguration(data, acpTestDigest(data), "test-model"); err == nil {
...
func TestACPStrictJSONAndFraming(t *testing.T) {
	for _, data := range []string{
		...
		var value any
		if acpDecode([]byte(data), &value, false) == nil {
			t.Fatal("ambiguous or unbounded JSON accepted")
		}
	}
	for _, raw := range []string{`"\ud83c\udf0d"`, `"\\ud800"`, `"\ufffd"`, `"héllo 🌍"`} {
		var value string
		if acpDecode([]byte(raw), &value, false) != nil {
...
func TestACPProtocolSelectionDoesNotInitializeAzure(t *testing.T) {
	...
	if handled, err := maybeServeACP(nil, in, nil); handled || err != nil {
		t.Fatal("legacy entry point was claimed")
	}
	if handled, err := maybeServeACP([]string{"--protocol", "unsupported"}, in, nil); !handled || err == nil {
