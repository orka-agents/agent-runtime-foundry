...
func newACPMCPClient(server acpMCPServer, client *http.Client) (*acpMCPClient, error) {
	if server.Type != "http" || !acpSafeString(server.Name, 128) || len(server.Headers) != 1 {
	...
	if !strings.EqualFold(header.Name, "Authorization") || !ok ||
		!acpSafeString(value, 16<<10) || strings.ContainsAny(value, " \t") {
		return nil, acpInvalidParams
	}
...
func (m *acpMCPClient) initialize(ctx context.Context) error {
	...
	if acpDecode(result, &reply, false) != nil || reply.ProtocolVersion != acpMCPVersion ||
...
func (m *acpMCPClient) tools(ctx context.Context) ([]foundryToolSchema, error) {
	...
	if acpDecode(result, &reply, false) != nil || reply.Tools == nil || len(reply.Tools) > defaultMaxBrokeredCalls ||
		(len(reply.NextCursor) != 0 && !bytes.Equal(reply.NextCursor, []byte("null"))) {
		return nil, errACPMCP
	}
	tools := make([]foundryToolSchema, 0, len(reply.Tools))
	seen := make(map[string]bool)
	for _, tool := range reply.Tools {
		if validateFoundryFunctionName(tool.Name) != nil || seen[tool.Name] || len(tool.InputSchema) > maxFoundryToolSchemaBytes {
			return nil, errACPMCP
		}
		var schema map[string]any
		if acpDecode(tool.InputSchema, &schema, false) != nil || schema == nil || schema["type"] != "object" {
			return nil, errACPMCP
		}
		seen[tool.Name] = true
		tools = append(tools, foundryToolSchema{Type: "function", Name: tool.Name, Description: tool.Description, Parameters: tool.InputSchema})
	}
	encoded, err := json.Marshal(tools)
	if err != nil || len(encoded) > maxFoundryToolSchemaBytes {
		return nil, errACPMCP
	}
	return tools, nil
}
...
func (m *acpMCPClient) execute(ctx context.Context, name string, args json.RawMessage) (string, bool, error) {
	...
	if acpDecode(result, &reply, false) != nil || reply.Content == nil {
...
func (m *acpMCPClient) call(ctx context.Context, method string, params any) (json.RawMessage, error) {
	...
	if err != nil || len(encoded) > defaultMaxBrokeredBytes {
		return nil, errACPMCP
	}
	...
	data, err := io.ReadAll(io.LimitReader(response.Body, defaultMaxBrokeredBytes+1))
	if err != nil || len(data) > defaultMaxBrokeredBytes {
		return nil, errACPMCP
	}
	var reply struct {
	...
	if acpDecode(data, &reply, true) != nil || reply.JSONRPC != "2.0" || !bytes.Equal(reply.ID, id) ||
