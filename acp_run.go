...
func (s *acpServer) runPrompt(ctx context.Context, session *acpSession, prompt string) (string, string, error) {
	...
	request := foundryResponseRequest{Input: prompt, PreviousResponseID: session.previous}
	if s.cfg.agent.ToolSchemaMode == toolSchemaModeRequest {
		request.Tools = tools
	}
	...
	for range acpMaxResponseRounds {
		...
		if err != nil || text.Len()+len(summary.Text) > defaultMaxOutputBytes {
		...
		if len(seen)+len(summary.FunctionCalls) > defaultMaxBrokeredCalls {
			return "", "", errACPProvider
		}
		// Validate the whole batch before admitting its first side effect.
		calls := summary.FunctionCalls
		for i, call := range calls {
			if validateFoundryFunctionName(call.Name) != nil || !allowed[call.Name] ||
				validateFoundryCallID(call.CallID) != nil || validateProviderIdentifier("call", call.CallID) != nil || seen[call.CallID] || len(call.Arguments) == 0 {
				return "", "", errACPProvider
			}
			arguments, err := normalizeFoundryToolArguments(call.Arguments)
			var object map[string]any
			if err != nil || len(arguments) > defaultMaxBrokeredBytes || acpDecode(arguments, &object, false) != nil || object == nil {
				return "", "", errACPProvider
			}
...
func (s *acpServer) executeTools(ctx context.Context, session *acpSession, calls []foundryOutputItem) ([]foundryFunctionOutput, error) {
	...
	defer cancel()
	outputs := make([]foundryFunctionOutput, len(calls))
	...
	for i, call := range calls {
		wg.Go(func() {
			...
			mu.Lock()
			total += len(output)
			if total > defaultMaxBrokeredTurnBytes && err == nil {
				err = errACPMCP
			}
			if err != nil {
				if firstError == nil {
			...
			outputs[i] = foundryFunctionOutput{Type: "function_call_output", CallID: call.CallID, Output: output}
		})
	}
	wg.Wait()
	if firstError != nil {
