...
func acpTestCompleted(w http.ResponseWriter, id, text string, calls ...foundryOutputItem) {
	output := append([]foundryOutputItem(nil), calls...)
	if text != "" {
		output = append(output, foundryOutputItem{Type: "message", Content: []foundryOutputContent{{Type: "output_text", Text: text}}})
	}
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(foundryResponse{ID: id, Status: "completed", Output: output})
}
