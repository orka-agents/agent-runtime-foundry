...
type scriptedResponse struct {
	validate func(foundryResponseRequest) error
	run      func(context.Context, responseCallbacks) (foundryStreamSummary, error)
}

type fakeResponsesBackend struct {
	mu              sync.Mutex
	responses       []scriptedResponse
	requests        []foundryResponseRequest
	createSessionID string
	createSessions  int
	validateErr     error
}

func (f *fakeResponsesBackend) CreateResponse(ctx context.Context, request foundryResponseRequest, callbacks responseCallbacks) (foundryStreamSummary, error) {
	f.mu.Lock()
	f.requests = append(f.requests, request)
	if len(f.responses) == 0 {
		f.mu.Unlock()
		return foundryStreamSummary{}, errors.New("no scripted response")
	}
	...
	backend := &fakeResponsesBackend{responses: []scriptedResponse{
		{
			...
			run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				if err := callbacks.OnCreated(foundryResponse{ID: "resp-tool", Status: "in_progress", AgentSessionID: "session-tool"}); err != nil {
					return foundryStreamSummary{}, err
				}
				call := foundryOutputItem{Type: "function_call", CallID: "call-1", Name: "lookup_ticket", Arguments: json.RawMessage(`"{\"ticket\":\"INC-1\"}"`)}
				if err := callbacks.OnFunctionCall(call); err != nil {
					return foundryStreamSummary{}, err
				}
				return foundryStreamSummary{ResponseID: "resp-tool", AgentSessionID: "session-tool", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
			},
		},
		{
			validate: func(request foundryResponseRequest) error {
...
func TestAdapterCancellationCancelsStreamAndResponse(t *testing.T) {
	...
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(ctx context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			if err := callbacks.OnCreated(foundryResponse{ID: "resp-cancel", Status: "in_progress"}); err != nil {
				return foundryStreamSummary{}, err
			}
			close(started)
			<-ctx.Done()
			return foundryStreamSummary{}, ctx.Err()
		},
	}}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestAdapterMapsFailedAndIncompleteResponses(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: "failed",
			script: scriptedResponse{run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-failed", Status: "in_progress"})
				return foundryStreamSummary{ResponseID: "resp-failed", Status: "failed", Error: &foundryError{Code: "server_error"}}, nil
			}},
		...
		},
		{
			name: "incomplete",
			script: scriptedResponse{run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-incomplete", Status: "in_progress"})
				return foundryStreamSummary{ResponseID: "resp-incomplete", Status: "incomplete", Incomplete: &foundryIncomplete{Reason: "max_output_tokens"}}, nil
			}},
...
func TestAdapterRejectsUnsafeToolCallAndOversizedResult(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-unsafe", Status: "in_progress"})
			if err := callbacks.OnFunctionCall(foundryOutputItem{Type: "function_call", CallID: "call-1", Name: "not_allowed", Arguments: json.RawMessage(`{}`)}); err != nil {
				return foundryStreamSummary{}, err
			}
			return foundryStreamSummary{ResponseID: "resp-unsafe", Status: "completed"}, nil
		},
	}}}
	cfg := testConfig("http://127.0.0.1")
...
func conformanceBackend(class harness.BrokeredToolClass) *fakeResponsesBackend {
	...
	return &fakeResponsesBackend{responses: []scriptedResponse{
		{
			run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-tool", Status: "in_progress", AgentSessionID: "session-tool"})
				call := foundryOutputItem{Type: "function_call", CallID: "call-1", Name: "conformance_" + string(class), Arguments: json.RawMessage(`{"value":"probe"}`)}
				if err := callbacks.OnFunctionCall(call); err != nil {
					return foundryStreamSummary{}, err
				}
				return foundryStreamSummary{ResponseID: "resp-tool", AgentSessionID: "session-tool", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
...
func textResponseScript(responseID, sessionID, text string) scriptedResponse {
	return scriptedResponse{run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
		if err := callbacks.OnCreated(foundryResponse{ID: responseID, Status: "in_progress", AgentSessionID: sessionID}); err != nil {
			return foundryStreamSummary{}, err
		}
		if err := callbacks.OnTextDelta(text); err != nil {
			return foundryStreamSummary{}, err
		}
		return foundryStreamSummary{ResponseID: responseID, AgentSessionID: sessionID, Status: "completed", Text: text}, nil
	}}
}
...
func TestAdapterDeadlineWhileWaitingForTool(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-wait", Status: "in_progress"})
			call := foundryOutputItem{Type: "function_call", CallID: "call-wait", Name: "lookup", Arguments: json.RawMessage(`{}`)}
			if err := callbacks.OnFunctionCall(call); err != nil {
				return foundryStreamSummary{}, err
			}
			return foundryStreamSummary{ResponseID: "resp-wait", AgentSessionID: "session-wait", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
		},
	}}}
	cfg := testConfig("http://127.0.0.1")
...
func TestAdapterContinuationRetryIsIdempotent(t *testing.T) {
	...
	backend := &fakeResponsesBackend{responses: []scriptedResponse{
		{
			run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-idem", Status: "in_progress"})
				call := foundryOutputItem{Type: "function_call", CallID: "call-idem", Name: "lookup", Arguments: json.RawMessage(`{}`)}
				if err := callbacks.OnFunctionCall(call); err != nil {
					return foundryStreamSummary{}, err
				}
				return foundryStreamSummary{ResponseID: "resp-idem", AgentSessionID: "session-idem", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
			},
		},
		{
			run: func(ctx context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-idem-final", Status: "in_progress"})
				select {
				case <-block:
				case <-ctx.Done():
					return foundryStreamSummary{}, ctx.Err()
				...
				return foundryStreamSummary{ResponseID: "resp-idem-final", Status: "completed", Text: "ok"}, nil
			},
		},
	}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestAdapterRejectsConcurrentTurnsAndPrunesCompletedState(t *testing.T) {
	...
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(ctx context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-active", Status: "in_progress"})
			select {
			case <-block:
			case <-ctx.Done():
				return foundryStreamSummary{}, ctx.Err()
			}
			return foundryStreamSummary{ResponseID: "resp-active", Status: "completed"}, nil
		},
	}}}
	cfg := testConfig("http://127.0.0.1")
	adapter := newAdapter(cfg, backend)
...
func TestAdapterQueuesEarlyToolResultUntilResponseCompletes(t *testing.T) {
	...
	backend := &fakeResponsesBackend{responses: []scriptedResponse{
		{
			run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-early", Status: "in_progress"})
				call := foundryOutputItem{Type: "function_call", CallID: "call-early", Name: "lookup", Arguments: json.RawMessage(`{}`)}
				if err := callbacks.OnFunctionCall(call); err != nil {
					return foundryStreamSummary{}, err
				}
				<-allowComplete
				return foundryStreamSummary{ResponseID: "resp-early", AgentSessionID: "session-early", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
			},
		},
...
func TestRetryableResponseFailureIsConservative(t *testing.T) {
	if !retryableResponseFailure(&foundryError{Code: "server_error"}) {
		t.Fatal("server_error should be retryable")
	}
	if retryableResponseFailure(&foundryError{Code: "content_filter"}) {
...
func TestUnresolvedToolResponseDoesNotAdvanceSessionCheckpoint(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-unresolved", Status: "in_progress", AgentSessionID: "session-1"})
			call := foundryOutputItem{Type: "function_call", CallID: "call-unresolved", Name: "lookup", Arguments: json.RawMessage(`{}`)}
			if err := callbacks.OnFunctionCall(call); err != nil {
				return foundryStreamSummary{}, err
			}
			return foundryStreamSummary{ResponseID: "resp-unresolved", AgentSessionID: "session-1", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
		},
	}}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestBrokeredTurnStateIsCumulativelyBounded(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-bounded", Status: "in_progress"})
			call := foundryOutputItem{Type: "function_call", CallID: "call-bounded", Name: "lookup", Arguments: json.RawMessage(`{"key":"value"}`)}
			if err := callbacks.OnFunctionCall(call); err != nil {
				return foundryStreamSummary{}, err
			}
			return foundryStreamSummary{ResponseID: "resp-bounded", AgentSessionID: "session-bounded", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
		},
	}}}
	cfg := testConfig("http://127.0.0.1")
...
func TestValidateProviderIdentifierBoundsAndRejectsWhitespace(t *testing.T) {
	if err := validateProviderIdentifier("response id", strings.Repeat("x", maxProviderIdentifierBytes+1)); err == nil {
		t.Fatal("oversized provider identifier was accepted")
	}
	if err := validateProviderIdentifier("response id", "response id"); err == nil {
...
func TestBrokeredWriteFailureIsNotRetryable(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-write-failure", Status: "in_progress"})
			if err := callbacks.OnFunctionCall(foundryOutputItem{Type: "function_call", CallID: "call-write", Name: "write_ticket", Arguments: json.RawMessage(`{}`)}); err != nil {
				return foundryStreamSummary{}, err
			}
			return foundryStreamSummary{}, errors.New("temporary provider failure")
		},
	}}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestCompletedContinuationMustAdvanceResponseID(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{
		{
			run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-tool-id", Status: "in_progress", AgentSessionID: "session-id"})
				call := foundryOutputItem{Type: "function_call", CallID: "call-id", Name: "lookup", Arguments: json.RawMessage(`{}`)}
				if err := callbacks.OnFunctionCall(call); err != nil {
					return foundryStreamSummary{}, err
				}
				return foundryStreamSummary{ResponseID: "resp-tool-id", AgentSessionID: "session-id", Status: "completed", FunctionCalls: []foundryOutputItem{call}}, nil
			},
		},
		{
			run: func(_ context.Context, _ responseCallbacks) (foundryStreamSummary, error) {
				return foundryStreamSummary{Status: "completed", AgentSessionID: "session-id"}, nil
			},
		},
	}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestProviderFailureBeforeFirstEventIsNotRetryable(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, _ responseCallbacks) (foundryStreamSummary, error) {
			return foundryStreamSummary{}, providerHTTPError{
...
func TestObservedFailureAfterStartIsNotRetryable(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-observed-failure", Status: "in_progress"})
			return foundryStreamSummary{}, providerHTTPError{StatusCode: http.StatusServiceUnavailable, Operation: "POST responses"}
		},
	}}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestCompletedContinuationMustUseNewResponseID(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{
		...
		{
			run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
				_ = callbacks.OnCreated(foundryResponse{ID: "resp-same", Status: "in_progress", AgentSessionID: "session-same"})
				return foundryStreamSummary{ResponseID: "resp-same", AgentSessionID: "session-same", Status: "completed"}, nil
			},
		},
	}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestRecordFunctionCallRejectsSubmittedCallIDReuse(t *testing.T) {
	...
	if err := adapter.recordFunctionCall(turn, foundryOutputItem{Type: "function_call", CallID: "call-reused", Name: "lookup", Arguments: json.RawMessage(`{}`)}); err == nil {
...
func TestAdapterPreservesOpaqueProviderCallIDs(t *testing.T) {
	...
	if err := adapter.recordFunctionCall(turn, foundryOutputItem{
		Type:      "function_call",
...
func TestEmptyExecutionModeIsNormalizedToObserved(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			_ = callbacks.OnCreated(foundryResponse{ID: "resp-empty-mode", Status: "in_progress"})
			return foundryStreamSummary{}, providerHTTPError{StatusCode: http.StatusServiceUnavailable, Operation: "POST responses"}
		},
	}}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
...
func TestTextDeltaSynthesizesTurnStartedFirst(t *testing.T) {
	backend := &fakeResponsesBackend{responses: []scriptedResponse{{
		run: func(_ context.Context, callbacks responseCallbacks) (foundryStreamSummary, error) {
			...
			return foundryStreamSummary{ResponseID: "resp-no-created", AgentSessionID: "session-no-created", Status: "completed", Text: "hello"}, nil
		},
	}}}
	adapter := newAdapter(testConfig("http://127.0.0.1"), backend)
