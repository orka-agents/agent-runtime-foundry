...
func brokerCapacityFillHistory(t *testing.T, b *lifecycleBroker, c brokerContext, target int, ordinary bool) {
	...
	err := b.commitCapacityLocked(ordinary, func(next *brokerLedger) error {
		...
		history := &brokerSession{Owner: owner, CreateState: "none", Retiring: true, Retired: true,
			ProofDigest: brokerSHA([]byte("synthetic-retained-proof")), Prompts: map[string]*brokerPrompt{},
		...
		size := len(data)
		digest := brokerSHA([]byte("synthetic-retained-operation"))
...
func TestBrokerByteReserveCoversEscapedAcceptanceAndCleanup(t *testing.T) {
	cfg := brokerConfiguration{configDigest: brokerSHA([]byte("capacity-bounds"))}
	c := brokerTestContext(cfg)
	c.BodySHA256 = brokerSHA([]byte("synthetic-body"))
	...
	if _, err := brokerRecordOperation(session, brokerResponsesPath, c); err != nil {
	...
	ledger.PrincipalDigest = brokerSHA([]byte("bound-principal"))
	...
	session.ProofDigest = brokerSHA([]byte("retirement-proof"))
	prompt.Closing, prompt.Settled = true, true
	prompt.ProofDigest = brokerSHA([]byte("settlement-proof"))
	invocation.ResponseID = strings.Repeat("<", maxProviderIdentifierBytes)
...
func TestBrokerByteCapacityRejectsBeforeIOAndKeepsFailureClosed(t *testing.T) {
	...
	defer cancel()
	b.active[brokerJSONDigest(c.Owner)] = brokerActive{prompt: c.promptKey(), cancel: cancel}
	...
	if err := b.control(brokerRenewPath, renewal); !errors.Is(err, errBrokerCapacity) {
	...
	err = b.commitLocked(func(next *brokerLedger) error {
		...
		for i := range 32 {
			...
			session.Operations[id] = brokerSHA([]byte("extra-operation"))
		}
		return nil
	})
	if !errors.Is(err, errBrokerCapacity) || b.storageError != nil || active.Err() != nil ||
		brokerSHA(before) != brokerJSONDigest(b.ledger) {
	...
	err = b.commitLocked(func(next *brokerLedger) error {
		next.Sessions[brokerJSONDigest(c.Owner)].Prompts[c.promptKey()].Closing = true
		return nil
	})
	...
	if brokerSHA(before) != brokerJSONDigest(b.ledger) {
...
func TestBrokerByteCapacityConcurrentOwnersPreserveAcceptanceAndCleanup(t *testing.T) {
	responseID := strings.Repeat("<", maxProviderIdentifierBytes)
	f := newBrokerEvidenceFixture(t, func(request foundryResponseRequest) (string, []byte) {
		response := foundryResponse{ID: responseID, AgentSessionID: request.AgentSessionID, Status: "completed"}
		for i := range defaultMaxBrokeredCalls {
			prefix := fmt.Sprintf("call-%03d-", i)
			response.Output = append(response.Output, foundryOutputItem{ID: fmt.Sprintf("item-%03d", i), Type: "function_call",
				Name: "hosted-probe-read", CallID: prefix + strings.Repeat("x", maxProviderIdentifierBytes-len(prefix)), Arguments: json.RawMessage(`"{}"`)})
		}
	...
	for _, c := range owners {
		if b.ledger.Sessions[brokerJSONDigest(c.Owner)].CreateState != "intent" {
			b.mu.Unlock()
	...
	for i, c := range owners {
		settlement, _ := brokerTestControlContext(brokerSettlePath, c)
		settlement.OperationID = strings.Repeat("<", 512)
		for range 3 {
			proof := brokerCapacityControlProof(t, server.URL, brokerSettlePath, settlement)
		...
		retirement, _ := brokerTestControlContext(brokerRetirePath, c)
		retirement.OperationID = strings.Repeat(">", 512)
		proof := brokerCapacityControlProof(t, server.URL, brokerRetirePath, retirement)
	...
	for i, c := range owners {
		retirement, _ := brokerTestControlContext(brokerRetirePath, c)
		retirement.OperationID = strings.Repeat(">", 512)
		proof := brokerCapacityControlProof(t, restarted.URL, brokerRetirePath, retirement)
		if !proof.RetirementProven || proof.ProofDigest != proofs[i] {
			t.Fatal("restart lost the original retirement receipt")
		}
		status, _, err := brokerTestHTTP(context.Background(), restarted.URL, brokerResponsesPath, c, brokerTestBody(""))
...
func TestBrokerByteCapacityLegacyLedgerStillRecovers(t *testing.T) {
	...
	status, _, err = brokerTestHTTP(context.Background(), restarted.URL, brokerResponsesPath, fresh, brokerTestBody(""))
	...
	reopened.mu.Lock()
	healthy := reopened.storageError == nil && reopened.ledger.Sessions[brokerJSONDigest(fresh.Owner)] == nil
	reopened.mu.Unlock()
	creates, inferences, stops, deletes := f.counts()
