...
func brokerFunctionBody(previous, call string) []byte {
	body, _ := json.Marshal(acpResponseRequest{Model: "fixture-model", foundryResponseRequest: foundryResponseRequest{
		Stream: true, Store: true, PreviousResponseID: previous,
		Input: []foundryFunctionOutput{{Type: "function_call_output", CallID: call, Output: "fixture-tool-result"}},
	}})
	return body
}
...
func TestBrokerOpaqueAliasesAndNoFunctionReplay(t *testing.T) {
	...
	for _, which := range []string{"duplicate", "native_call", "cross_owner", "cross_prompt", "missing_output"} {
		...
		status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerResponsesPath, bad, body)
	...
	status, data, err = brokerTestHTTP(context.Background(), server.URL, brokerResponsesPath, next, brokerFunctionBody(first.ID, call))
	var second foundryResponse
	...
	for _, previous := range []string{first.ID, second.ID} {
		...
		status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerResponsesPath, replay, brokerFunctionBody(previous, call))
		if err != nil || status != http.StatusConflict {
	...
	f.mu.Unlock()
	_ = brokerTestControl(t, server.URL, brokerSettlePath, next)
	_ = brokerTestControl(t, server.URL, brokerRetirePath, next)
}
...
func TestBrokerControlProofBindsExactHeaderBytes(t *testing.T) {
	...
	cfg := brokerTestConfig(t, f)
	_, server := startBrokerTest(t, cfg)
	c, body := brokerTestControlContext(brokerSettlePath, brokerTestContext(cfg))
	c.BodySHA256 = brokerSHA(body)
	status, data, err := brokerTestHTTP(context.Background(), server.URL, brokerSettlePath, c, body)
	...
	if proof.Protocol != brokerProtocol || proof.OwnerDigest != brokerJSONDigest(c.Owner) ||
		proof.OperationID != c.OperationID || proof.ContextSHA256 != brokerJSONDigest(c) {
	...
	c.LeaseGeneration++
	status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerSettlePath, c, body)
	if err != nil || status != http.StatusConflict {
