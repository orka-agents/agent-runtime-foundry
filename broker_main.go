...
func maybeServeBroker(args []string) (bool, error) {
	selected := false
	...
	if *healthCheck {
		return true, checkBrokerHealth(firstNonBlank(os.Getenv("ORKA_FOUNDRY_BROKER_ADDR"), "127.0.0.1:8091"))
	}
...
func loadBrokerConfiguration(path string, getenv func(string) string) (brokerConfiguration, error) {
	...
	digest := getenv(acpConfigDigestEnv)
	agent, err := decodeACPAgentConfiguration(data, digest, getenv(acpModelEnv))
	if err != nil {
		return brokerConfiguration{}, errBrokerInvalid
	}
	cfg := brokerConfiguration{agent: agent, configDigest: digest,
		addr:     firstNonBlank(getenv("ORKA_FOUNDRY_BROKER_ADDR"), "127.0.0.1:8091"),
	...
	if !brokerAddressValid(cfg.addr) || cfg.stateDir == "" ||
		!acpSafeString(cfg.bearer, 16<<10) || len(cfg.bearer) < 32 || strings.ContainsAny(cfg.bearer, " \t") ||
		(getenv(envIsolationMode) != "" && getenv(envIsolationMode) != "entra") {
		return brokerConfiguration{}, errBrokerInvalid
	}
	return cfg, nil
}
