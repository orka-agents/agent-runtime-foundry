...
func brokerAsyncInference(ctx context.Context, base string, c brokerContext, body []byte) <-chan brokerHTTPResult {
	...
	go func() {
		status, data, err := brokerTestHTTP(ctx, base, brokerResponsesPath, c, body)
...
func brokerInvocationState(b *lifecycleBroker, c brokerContext) string {
	b.mu.Lock()
	defer b.mu.Unlock()
	if session := b.ledger.Sessions[brokerJSONDigest(c.Owner)]; session != nil {
...
func TestBrokerCancellationWaitsForCreateAcknowledgement(t *testing.T) {
	...
	brokerPendingControl(t, server.URL, brokerSettlePath, c, true, 0)
	...
	owned := ledger.Sessions[brokerJSONDigest(c.Owner)]
	...
	if !proof.RetirementProven || !brokerDigestValid(proof.ProofDigest) {
