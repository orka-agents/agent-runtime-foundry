...
type brokerRemoteSession struct {
...
func (b *lifecycleBroker) pinPrincipal(token string) error {
	...
	if err != nil || acpDecodeStruct(raw, &claims, false) != nil || strings.TrimRight(claims.Audience, "/") != "https://ai.azure.com" ||
		!acpSafeString(claims.Tenant, 512) || !acpSafeString(claims.Object, 512) {
		return errBrokerRemote
	}
	digest := brokerJSONDigest(claims)
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *lifecycleBroker) remoteJSONRequest(request *http.Request, target any) (int, error) {
	...
	data, err := io.ReadAll(io.LimitReader(response.Body, acpMaxConfigBytes+1))
	...
	if target != nil && response.StatusCode >= 200 && response.StatusCode < 300 {
		if acpDecodeStruct(data, target, false) != nil {
			return response.StatusCode, errBrokerRemote
		}
	}
...
func (b *lifecycleBroker) validateRemoteTarget(ctx context.Context) error {
	...
	for _, scheme := range agent.Endpoint.Schemes {
		...
		if acpDecodeStruct(scheme, &value, true) != nil || !strings.EqualFold(value.Type, "entra") {
			return errBrokerRemote
		}
	}
	var version struct {
...
func (b *lifecycleBroker) remoteSessionMatches(value brokerRemoteSession, id string) bool {
	return value.ID == id && value.Version.Type == "version_ref" && value.Version.Version == b.cfg.agent.HostedTarget.AgentVersion
}

func brokerSessionSuffix(id string) string { return "/endpoint/sessions/" + url.PathEscape(id) }

func (b *lifecycleBroker) remoteSessionGet(ctx context.Context, id string) (brokerRemoteSession, int, error) {
	var session brokerRemoteSession
	status, err := b.remoteJSON(ctx, http.MethodGet, brokerSessionSuffix(id), nil, &session)
...
func (b *lifecycleBroker) remoteSessionCreate(request *http.Request, id string) (bool, error) {
	...
	if err == nil && brokerDefiniteRejection(status) {
		return false, nil
	}
...
func (b *lifecycleBroker) remoteSessionStop(ctx context.Context, id string) error {
	...
	status, err = b.remoteJSON(ctx, http.MethodPost, brokerSessionSuffix(id)+":stop", nil, nil)
...
func (b *lifecycleBroker) remoteSessionDelete(ctx context.Context, id string) error {
	...
	status, err = b.remoteJSON(ctx, http.MethodDelete, brokerSessionSuffix(id), nil, nil)
