...
func requireBlockedRenewalReplay(t *testing.T, base string, renewal brokerContext, pending bool, ambiguous uint32) {
	t.Helper()
	proof := brokerTestControl(t, base, brokerRenewPath, renewal)
...
func TestBrokerRenewalDuringPendingCreateSurvivesOriginalExpiry(t *testing.T) {
	...
	proof := brokerTestControl(t, server.URL, brokerRenewPath, renewal)
	...
	requirePendingCreateRenewal(t, brokerTestControl(t, server.URL, brokerRenewPath, renewal), renewal)
	status := brokerTestControl(t, server.URL, brokerStatusPath, c)
	...
	b.mu.Lock()
	prompt := b.ledger.Sessions[brokerJSONDigest(c.Owner)].Prompts[c.promptKey()]
	...
	for _, path := range []string{brokerSettlePath, brokerRetirePath} {
		t.Run(path, func(t *testing.T) {
			...
			if path == brokerSettlePath {
				...
				cc, body := brokerTestControlContext(brokerRenewPath, renewal)
				status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerRenewPath, cc, body)
