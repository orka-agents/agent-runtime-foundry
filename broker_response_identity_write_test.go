...
func newBrokerResponseIdentityFixture(t *testing.T) (*lifecycleBroker, brokerContext) {
	t.Helper()
	cfg := brokerConfiguration{configDigest: brokerSHA([]byte("identity-write-fixture")), stateDir: filepath.Join(t.TempDir(), "broker")}
	...
	c.BodySHA256 = brokerSHA([]byte("identity-write-input"))
	...
	err = b.commitLocked(func(next *brokerLedger) error {
		...
		next.PrincipalDigest = brokerSHA([]byte("identity-write-principal"))
...
func brokerIdentityLedgerBytes(t *testing.T, b *lifecycleBroker) []byte {
	...
	if json.Unmarshal(data, &ledger) != nil || !brokerLedgerValid(&ledger, b.ledger.ConfigDigest) ||
		brokerJSONDigest(ledger) != brokerJSONDigest(b.ledger) {
...
func TestBrokerResponseIdentityWritesOncePerInvocation(t *testing.T) {
	for _, test := range []struct {
		...
		{"repeated", 64, true},
		{"event-limit", defaultMaxEvents, true},
		{"over-event-limit", defaultMaxEvents + 1, false},
	} {
		t.Run(test.name, func(t *testing.T) {
			b, c := newBrokerResponseIdentityFixture(t)
			events := make([]string, test.events)
			for i := range events {
				events[i] = acpTestSSE(`{"type":"response.in_progress","response":{"id":"response-1","status":"in_progress"}}`)
			}
			events[0] = acpTestSSE(`{"type":"response.created","response":{"id":"response-1","status":"in_progress"}}`)
			events[len(events)-1] = acpTestSSE(`{"type":"response.completed","response":{"id":"response-1","status":"completed"}}`)
			...
			invocation := b.ledger.Sessions[brokerJSONDigest(c.Owner)].Prompts[c.promptKey()].Invocations[c.InvocationSequence]
...
func TestBrokerResponseIdentityDuplicateStillRejectsConflicts(t *testing.T) {
	b, c := newBrokerResponseIdentityFixture(t)
	accepted := foundryResponse{ID: "response-1", AgentSessionID: brokerIdentityRemoteSession}
	...
	for name, response := range map[string]foundryResponse{
	...
	if _, err := b.commitCompletedResponse(c, foundryStreamSummary{ResponseID: accepted.ID}); err != nil {
	...
	if err := b.commitLocked(func(next *brokerLedger) error {
		...
		if _, err := brokerRecordOperation(session, brokerResponsesPath, nextContext); err != nil {
			return err
		}
...
func TestBrokerResponseIdentityConcurrentDuplicatesDoNotWrite(t *testing.T) {
	b, c := newBrokerResponseIdentityFixture(t)
	response := foundryResponse{ID: "response-1"}
...
func TestBrokerResponseIdentityTrackingStillRejectsMalformedTail(t *testing.T) {
	b, c := newBrokerResponseIdentityFixture(t)
	created := acpTestSSE(`{"type":"response.created","response":{"id":"response-1","status":"in_progress"}}`)
	stream := created + acpTestSSE(`{"type":"response.in_progress","response":{"id":"response-1","status":"completed"}}`)
