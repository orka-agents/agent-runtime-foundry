...
func brokerCaseRequest(t *testing.T, b *lifecycleBroker, c brokerContext, members string) *httptest.ResponseRecorder {
	...
	request := httptest.NewRequest(http.MethodPost, brokerResponsesPath, bytes.NewReader(body))
...
func brokerCaseRejectBeforeOwnership(t *testing.T, mode, members string) {
	...
	cfg.configDigest = brokerJSONDigest(cfg.agent)
...
func TestBrokerProtectedFieldsRejectDecoderCaseVariantsBeforeOwnership(t *testing.T) {
	fields := []struct {
		...
		{"tools", []string{`"tools"`, `"Tools"`, `"TOOLS"`, `"tOoLs"`, `"toolſ"`, `"TOOLſ"`, `"\u0054ools"`, `"tool\u017f"`},
			[]string{brokerCaseToolSchema, `[]`, `null`}, []string{toolSchemaModeProviderStatic}},
		{"agent_session_id", []string{`"agent_session_id"`, `"Agent_Session_ID"`, `"AGENT_SESSION_ID"`, `"aGeNt_sEsSiOn_iD"`, `"agent_ſeſſion_id"`, `"\u0041gent_session_id"`, `"agent_\u017fe\u017f\u017fion_id"`},
			[]string{`"fixture-injected-session"`, `""`, `null`}, []string{toolSchemaModeProviderStatic, toolSchemaModeRequest}},
	}
...
func TestBrokerProtectedFieldsRejectDuplicatePresenceBeforeOwnership(t *testing.T) {
	for _, tc := range []struct {
		...
		{"tools_folded_null_last", `"Tools":` + brokerCaseToolSchema + `,"TOOLS":null`, []string{toolSchemaModeProviderStatic}},
		{"tools_folded_value_last", `"Tools":null,"TOOLS":` + brokerCaseToolSchema, []string{toolSchemaModeProviderStatic}},
		{"tools_folded_empty_last", `"Tools":` + brokerCaseToolSchema + `,"TOOLſ":[]`, []string{toolSchemaModeProviderStatic}},
		{"tools_canonical_null", `"tools":null,"Tools":` + brokerCaseToolSchema, []string{toolSchemaModeProviderStatic}},
		{"tools_exact_duplicate", `"Tools":null,"Tools":[]`, []string{toolSchemaModeProviderStatic}},
		{"tools_escaped_duplicate", `"Tools":null,"\u0054ools":[]`, []string{toolSchemaModeProviderStatic}},
		{"session_folded_null_last", `"Agent_Session_ID":"fixture-injected-session","AGENT_SESSION_ID":null`, []string{toolSchemaModeProviderStatic, toolSchemaModeRequest}},
		{"session_folded_value_last", `"Agent_Session_ID":null,"AGENT_SESSION_ID":"fixture-injected-session"`, []string{toolSchemaModeProviderStatic, toolSchemaModeRequest}},
		{"session_folded_empty_last", `"Agent_Session_ID":"fixture-injected-session","agent_ſeſſion_id":""`, []string{toolSchemaModeProviderStatic, toolSchemaModeRequest}},
		{"session_canonical_null", `"agent_session_id":null,"Agent_Session_ID":""`, []string{toolSchemaModeProviderStatic, toolSchemaModeRequest}},
		{"session_exact_duplicate", `"Agent_Session_ID":null,"Agent_Session_ID":""`, []string{toolSchemaModeProviderStatic, toolSchemaModeRequest}},
		{"session_escaped_duplicate", `"Agent_Session_ID":null,"\u0041gent_Session_ID":""`, []string{toolSchemaModeProviderStatic, toolSchemaModeRequest}},
	} {
...
func TestBrokerRequestToolCaseVariantsRemainValid(t *testing.T) {
	for _, tc := range []struct {
		...
		{"static_omitted", toolSchemaModeProviderStatic, "", 0},
		{"request_omitted", toolSchemaModeRequest, "", 0},
		{"canonical", toolSchemaModeRequest, `"tools":` + brokerCaseToolSchema, 1},
		{"title", toolSchemaModeRequest, `"Tools":` + brokerCaseToolSchema, 1},
		{"upper", toolSchemaModeRequest, `"TOOLS":` + brokerCaseToolSchema, 1},
		{"mixed", toolSchemaModeRequest, `"tOoLs":` + brokerCaseToolSchema, 1},
		{"unicode_fold", toolSchemaModeRequest, `"toolſ":` + brokerCaseToolSchema, 1},
		{"escaped", toolSchemaModeRequest, `"\u0054ools":` + brokerCaseToolSchema, 1},
		{"escaped_unicode_fold", toolSchemaModeRequest, `"tool\u017f":` + brokerCaseToolSchema, 1},
		{"empty", toolSchemaModeRequest, `"Tools":[]`, 0},
		{"null", toolSchemaModeRequest, `"TOOLS":null`, 0},
	} {
