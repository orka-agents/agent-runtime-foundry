	...
	for _, phase := range []string{"create", "stop", "delete"} {
		t.Run(phase, func(t *testing.T) {
			...
			if phase == "stop" {
				...
				b.closePrompt(brokerJSONDigest(c.Owner), c.promptKey())
				...
				brokerPendingControl(t, server.URL, brokerRetirePath, c, false, 0)
			}
			...
			other, body := brokerTestControlContext(brokerRenewPath, other)
			status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerRenewPath, other, body)
