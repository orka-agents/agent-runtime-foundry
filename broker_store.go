...
func openBrokerStore(dir, digest string) (*brokerStore, *brokerLedger, error) {
	if !filepath.IsAbs(dir) || filepath.Clean(dir) == string(filepath.Separator) || !brokerDigestValid(digest) {
		return nil, nil, errBrokerStorage
	}
	lock, created, err := openStoreLock(dir, "broker.lock", syncStoreDirectory)
	if err != nil {
		return nil, nil, errBrokerStorage
	}
	...
	if err != nil || !brokerPrivateFile(info) || info.Size() > brokerMaxLedgerBytes {
		store.close()
		return nil, nil, errBrokerStorage
	}
	data, err := os.ReadFile(path)
	if err != nil || acpDecode(data, ledger, true) != nil || !brokerLedgerValid(ledger, digest) {
		store.close()
		return nil, nil, errBrokerStorage
	}
	return store, ledger, nil
...
func brokerPrivateFile(info os.FileInfo) bool {
