	...
	if suffix == "/endpoint/sessions" && r.Method == http.MethodPost {
		...
		if strings.HasPrefix(f.mode, "create-rejected") {
			...
			if f.mode == "create-rejected-oversized" {
				_, _ = io.WriteString(w, strings.Repeat("x", acpMaxConfigBytes+1))
				return
			}
...
func brokerTestConfig(t *testing.T, fixture *brokerFixture) brokerConfiguration {
	t.Helper()
	agent := acpAgentConfiguration{Model: "fixture-model", ToolSchemaMode: toolSchemaModeProviderStatic,
		HostedTarget: acpHostedTarget{ProjectEndpoint: fixture.server.URL + "/api/projects/fixture", AgentName: "fixture", AgentVersion: "3"}}
	raw, _ := json.Marshal(agent)
	return brokerConfiguration{agent: agent, configDigest: brokerSHA(raw), stateDir: filepath.Join(t.TempDir(), "broker"), bearer: brokerFixtureBearer, operationTimeout: 2 * time.Second}
}
...
func brokerTestBody(previous string) []byte {
	request := acpResponseRequest{Model: "fixture-model", foundryResponseRequest: foundryResponseRequest{
...
func brokerTestControlContext(path string, c brokerContext) (brokerContext, []byte) {
	...
	if path == brokerRetirePath || path == brokerStatusPath {
		c.TaskUID = ""
		c.TaskAttempt = 0
		c.PromptID = ""
...
func TestBrokerLifecycleOwnershipContinuationAndRetirement(t *testing.T) {
	...
	if !proof.SettlementProven || proof.ActiveInvocations != 0 || proof.AmbiguousInvocations != 0 || !brokerDigestValid(proof.ProofDigest) {
	...
	duplicate := brokerTestControl(t, server.URL, brokerRetirePath, c)
	...
	b.mu.Lock()
	retired := b.ledger.Sessions[brokerJSONDigest(c.Owner)].Retired
	b.mu.Unlock()
	if !retired {
...
func TestBrokerRenewBeforeInference(t *testing.T) {
	...
	c := brokerTestContext(cfg)
	c.LeaseGeneration = 4
	proof := brokerTestControl(t, server.URL, brokerRenewPath, c)
	...
	cc.BodySHA256 = brokerSHA([]byte("{}"))
	status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerRenewPath, cc, []byte("{}"))
	if err != nil || status != 200 {
...
func TestBrokerUnknownInferenceNeverClaimsCleanup(t *testing.T) {
	...
	go func() {
		defer close(done)
		_, _, _ = brokerTestHTTP(ctx, server.URL, brokerResponsesPath, c, brokerTestBody(""))
	}()
	select {
	case <-f.started:
	...
	brokerAwait(t, func() bool {
		b.mu.Lock()
		defer b.mu.Unlock()
		p := b.ledger.Sessions[brokerJSONDigest(c.Owner)].Prompts[c.promptKey()]
	...
	for range 3 {
		status, data, err := brokerTestHTTP(context.Background(), server.URL, brokerSettlePath, cc, []byte("{}"))
		var proof brokerControlResponse
	...
	status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerRetirePath, cc, []byte("{}"))
