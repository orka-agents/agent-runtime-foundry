		...
		t.Run(tc.name, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				agent := acpAgentConfiguration{Model: "fixture-model", ToolSchemaMode: toolSchemaModeProviderStatic,
					HostedTarget: acpHostedTarget{ProjectEndpoint: "http://synthetic.invalid/api/projects/fixture", AgentName: "fixture", AgentVersion: "3"}}
				raw, _ := json.Marshal(agent)
				cfg := brokerConfiguration{agent: agent, configDigest: brokerSHA(raw), stateDir: filepath.Join(t.TempDir(), "broker"), bearer: brokerFixtureBearer, operationTimeout: 45 * time.Second}
				...
				go func() { done <- brokerDeadlineRequest(b, brokerResponsesPath, c, brokerTestBody("")) }()
				for generation := uint64(2); generation <= 3; generation++ {
					...
					w := brokerDeadlineRequest(b, brokerRenewPath, renew, []byte("{}"))
					var proof brokerControlResponse
				...
				if tc.wantAmbiguity {
					brokerAwait(t, func() bool {
						b.mu.Lock()
						defer b.mu.Unlock()
						return b.ledger.Sessions[brokerJSONDigest(c.Owner)].Prompts[c.promptKey()].Invocations[1].State == "uncertain"
					})
				}
				...
				if pending {
					...
					proof := brokerDeadlineControl(t, b, brokerSettlePath, c)
					if !proof.SettlementProven {
						t.Fatal("exact synthetic prompt settlement missing")
					}
					proof = brokerDeadlineControl(t, b, brokerRetirePath, c)
