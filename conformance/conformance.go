...
import (
	...
	"github.com/orka-agents/agent-runtime-foundry/internal/events"
...
func Check(ctx context.Context, target Target) Result {
	...
	if err != nil {
		...
		result.ObservedCapabilities = sanitizeCapabilities(caps)
	}
...
func runReadinessProbes(
	...
	if observed {
		runTurnProbe(ctx, readinessTarget(target, "observed"), result, baseURL, controlTimeout)
	}
	if brokered {
...
func readinessTarget(target Target, profile string) Target {
	...
	request.Deadline = time.Now().UTC().Add(defaultProbeTimeout)
...
func runAuthProbe(ctx context.Context, target Target, result *Result, baseURL string, controlTimeout time.Duration) {
	...
	probe := unauthenticatedStartProbe(request)
	assertUnauthenticatedTurnResourcesRejected(ctx, target, result, baseURL, controlTimeout, probe)
}
...
func runTurnProbe(ctx context.Context, target Target, result *Result, baseURL string, controlTimeout time.Duration) {
	...
	if err := validateEventStreamPath(request.TurnID, started.EventStreamPath); err != nil {
...
func validateEventStreamPath(turnID harness.HarnessTurnID, actual string) error {
	expected, err := harness.EventStreamPath(turnID)
	if err != nil {
		return err
	}
	if strings.TrimSpace(actual) == "" {
		return errors.New("start turn response eventStreamPath is required")
	}
	if actual != expected {
		return errors.New("start turn response eventStreamPath does not match requested turn")
	}
	return nil
}
...
func runBrokeredProbe(
	...
	recordFrame := func(frame harness.HarnessEventFrame) bool {
		...
		if frame.Type == harness.FrameToolCallRequested {
			result.addFailure("brokered runtime requested an additional tool after continue")
			return false
		}
...
func streamBrokeredContinuationFrames(
	...
	err := client.StreamFrames(streamCtx, request.TurnID, afterSeq, func(frame harness.HarnessEventFrame) error {
		...
		frames = append(frames, frame)
		if frame.Type == harness.FrameToolCallRequested {
			return errors.New("brokered runtime requested an additional tool after continue")
		}
		if frame.Type == harness.FrameToolResultReceived && frame.ToolCallID == requested.ToolCallID {
			toolResultSeen = true
		}
	...
	for {
		...
		case frame := <-framesCh:
			*frames = append(*frames, frame)
			...
			if frame.Type == harness.FrameToolCallRequested {
				result.addFailure("brokered runtime requested more than one tool before continue")
				return false
			}
...
func isDuplicateStartRejectedError(err error) bool {
	...
	return errors.As(err, &clientErr) && clientErr.StatusCode == http.StatusConflict
}
...
func unauthenticatedProbeTokens(validToken string) []string {
	invalidToken := "orka-conformance-invalid-" + uuid.NewString()
	if invalidToken == strings.TrimSpace(validToken) {
		invalidToken += "-other"
	}
...
func assertUnauthenticatedContinueRejected(
	...
	for _, token := range unauthenticatedProbeTokens(target.BearerToken) {
		client, err := newClient(baseURL, token, target.HTTPClient, controlTimeout)
		if err != nil {
			result.addFailure(fmt.Sprintf("create invalid-auth client: %v", err))
			return false
		}
		if _, err := client.ContinueTurn(ctx, request); err == nil {
			result.addFailure("invalid-auth continue turn was accepted")
			return false
		} else if !isAuthRequiredError(err) {
			result.addFailure(fmt.Sprintf("invalid-auth continue turn returned %v, want 401/403", err))
			return false
		}
	}
	return true
...
func unauthenticatedStartProbe(request harness.StartTurnRequest) harness.StartTurnRequest {
	probe := defaultStartTurnRequest("conformance-unauthenticated-start")
	probe.ToolExecutionMode = request.ToolExecutionMode
	probe.Input.Tools = make([]harness.ToolDefinition, len(request.Input.Tools))
	copy(probe.Input.Tools, request.Input.Tools)
	for i := range probe.Input.Tools {
		probe.Input.Tools[i].Parameters = slices.Clone(request.Input.Tools[i].Parameters)
	}
	probe.Input.Prompt = "Orka unauthenticated conformance probe. Do not perform external actions."
	return probe
}
...
func assertUnauthenticatedStartRejected(
	...
	for _, token := range unauthenticatedProbeTokens(target.BearerToken) {
		...
		probe := unauthenticatedStartProbe(request)
		if _, err := client.StartTurn(ctx, probe); err == nil {
			if authorized, clientErr := newClient(baseURL, target.BearerToken, target.HTTPClient, controlTimeout); clientErr == nil {
				cancelProbeTurn(ctx, authorized, result, probe, "invalid-auth conformance start was accepted")
			}
			result.addFailure("invalid-auth start turn was accepted")
			return false
		} else if !isAuthRequiredError(err) {
			if startTurnMayHaveBeenAccepted(err) {
				if authorized, clientErr := newClient(baseURL, target.BearerToken, target.HTTPClient, controlTimeout); clientErr == nil {
					bestEffortCancelProbeTurn(ctx, authorized, probe, "invalid-auth conformance start may have been accepted")
				}
			}
			result.addFailure(fmt.Sprintf("invalid-auth start turn returned %v, want 401/403", err))
			return false
		}
	}
	return true
...
func assertUnauthenticatedTurnResourcesRejected(
	...
	for _, token := range unauthenticatedProbeTokens(target.BearerToken) {
		...
		streamCtx, cancel := context.WithTimeout(ctx, controlTimeout)
		err = client.StreamFrames(streamCtx, request.TurnID, 0, func(harness.HarnessEventFrame) error { return nil })
		cancel()
		if err == nil {
			result.addFailure("invalid-auth event stream was accepted")
		} else if !isAuthRequiredError(err) {
			result.addFailure(fmt.Sprintf("invalid-auth event stream returned %v, want 401/403", err))
		}
		if result.ObservedCapabilities != nil && result.ObservedCapabilities.SupportsContinuation {
			toolCallID := "conformance-auth-call"
			continueRequest := harness.ContinueTurnRequest{
				Version:          harness.ProtocolVersion,
				Namespace:        request.Namespace,
				TaskName:         request.TaskName,
				...
				ToolResults: []harness.ToolCallResult{{
					...
					IdempotencyKey: harness.ToolRequestIdempotencyKey(
						request.RuntimeSessionID, request.TurnID, toolCallID,
					),
					Approved: true,
					Output:   json.RawMessage(`{"ok":true}`),
				}},
			}
			if _, continueErr := client.ContinueTurn(ctx, continueRequest); continueErr == nil {
				result.addFailure("invalid-auth continue turn was accepted")
			} else if !isAuthRequiredError(continueErr) {
				result.addFailure(fmt.Sprintf("invalid-auth continue turn returned %v, want 401/403", continueErr))
			}
		}
		if result.ObservedCapabilities == nil || !result.ObservedCapabilities.SupportsCancel {
			continue
		}
		_, cancelErr := client.CancelTurn(ctx, harness.CancelTurnRequest{
			Version:          harness.ProtocolVersion,
			Namespace:        request.Namespace,
			TaskName:         request.TaskName,
			SessionName:      request.SessionName,
			...
			TurnID:           request.TurnID,
			CorrelationID:    request.CorrelationID,
			Reason:           "conformance invalid-auth cancel probe",
		})
		if cancelErr == nil {
			result.addFailure("invalid-auth cancel turn was accepted")
		} else if !isAuthRequiredError(cancelErr) {
			result.addFailure(fmt.Sprintf("invalid-auth cancel turn returned %v, want 401/403", cancelErr))
		}
	}
}
...
func startTurnMayHaveBeenAccepted(err error) bool {
	...
	if clientErr.RemoteAccepted || clientErr.StatusCode >= http.StatusOK && clientErr.StatusCode < http.StatusMultipleChoices {
		return true
	}
	return clientErr.Op == "post" && clientErr.StatusCode == 0
}
...
func validateProbeFrames(result *Result, request harness.StartTurnRequest, frames []harness.HarnessEventFrame) bool {
	...
	for i, frame := range frames {
		...
		if frame.Type != harness.FrameTurnStarted && !startedSeen {
			result.addFailure(fmt.Sprintf("frame %d type %q appears before TurnStarted", i, frame.Type))
		}
		switch frame.Type {
		case harness.FrameTurnStarted:
			started++
			startedSeen = true
	...
	if started != 1 {
		result.addFailure(fmt.Sprintf("TurnStarted frame count = %d, want exactly 1", started))
	}
	if terminal != 1 {
	...
	return started == 1 && terminal == 1 && completed == 1
}
...
func sanitizeCapabilities(caps *harness.CapabilitiesResponse) *harness.CapabilitiesResponse {
	if caps == nil {
		return nil
	}
	copyCaps := *caps
	copyCaps.RuntimeName = sanitizeCapabilityText(caps.RuntimeName)
	copyCaps.RuntimeVersion = sanitizeCapabilityText(caps.RuntimeVersion)
	copyCaps.ToolExecutionModes = slices.Clone(caps.ToolExecutionModes)
	copyCaps.BrokeredToolClasses = slices.Clone(caps.BrokeredToolClasses)
	copyCaps.Metadata = nil
	return &copyCaps
}

func sanitizeCapabilityText(value string) string {
	value = events.RedactExecutionEventText(strings.TrimSpace(value))
	runes := []rune(value)
	const maxCapabilityTextRunes = 256
	if len(runes) > maxCapabilityTextRunes {
		return string(runes[:maxCapabilityTextRunes])
	}
	return value
}
