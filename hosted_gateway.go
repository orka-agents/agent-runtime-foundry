...
func newHostedGateway(ctx context.Context, settings hostedGatewaySettings, provider foundryTokenProvider) (*hostedGateway, error) {
...
func (g *hostedGateway) initialize(settings hostedGatewaySettings) error {
	...
	bootstrapDigest, pairID := brokerSHA(body), uuid.NewString()
...
func (g *hostedGateway) validateCapabilities(ctx context.Context, transport http.RoundTripper) error {
	...
	if err != nil || len(data) > hostedMaxHandshakeBytes || response.StatusCode != http.StatusOK ||
		acpDecode(data, &value, false) != nil || value.Protocol != "orka.harness.v2" || value.Transport != "http+ndjson" ||
