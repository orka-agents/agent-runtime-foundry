...
type hostedImageConfig struct {
	...
	Target                   acpHostedTarget `json:"target"`
...
func validateHostedImageConfig(cfg hostedImageConfig) error {
	if cfg.Protocol != hostedProtocol || !hostedUUIDValid(cfg.DeploymentID) ||
		!brokerDigestValid(cfg.AgentConfigurationDigest) || !hostedTargetValid(cfg.Target) {
...
func hostedHelloSigningBytes(value hostedHello) ([]byte, error) {
	...
	if c.Protocol != hostedProtocol || !hostedUUIDValid(c.DeploymentID) ||
		!brokerDigestValid(c.ConfigurationDigest) || validateAgentName(c.AgentName) != nil ||
		...
		!brokerDigestValid(value.BootstrapDigest) || value.ExpiresAt <= 0 {
...
func validateHostedBootstrap(cfg hostedImageConfig, value hostedBootstrap) error {
	...
	for key, value := range value.Environment {
		...
		case "ORKA_ACP_PROVIDER":
			valid = value == "foundry"
		case "ORKA_ACP_MODEL":
			valid = utf8.ValidString(value) && acpSafeString(value, 512) && strings.TrimSpace(value) == value
...
func hostedTargetValid(target acpHostedTarget) bool {
	if validateAgentName(target.AgentName) != nil || !hostedPositiveUint(target.AgentVersion) ||
		!acpSafeString(target.ProjectEndpoint, 2048) || strings.ContainsAny(target.ProjectEndpoint, "%#") {
		return false
	}
...
func hostedCredentialValid(value string) bool {
	if len(value) < 32 || !utf8.ValidString(value) || !acpSafeString(value, 16<<10) {
		return false
	}
