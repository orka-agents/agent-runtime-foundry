...
func (g *hostedGateway) accessToken(ctx context.Context) (string, error) {
	...
	if err != nil || acpDecodeStruct(raw, &claims, false) != nil ||
...
func (g *hostedGateway) sendRemoteJSON(request *http.Request, target any) (int, error) {
	...
	if target != nil && response.StatusCode >= 200 && response.StatusCode < 300 && acpDecodeStruct(data, target, false) != nil {
		return response.StatusCode, errHostedInvalid
	}
...
func (g *hostedGateway) ensureSession(ctx context.Context) error {
	...
	status, err := g.remoteJSON(ctx, http.MethodGet, brokerSessionSuffix(g.cfg.SessionID), nil, &session)
	if err != nil {
		return err
	}
...
func (g *hostedGateway) sessionMatches(value brokerRemoteSession) bool {
...
func (g *hostedGateway) openChannel(ctx context.Context, role string) (*websocket.Conn, hostedChallenge, error) {
	...
	if readHostedWSJSON(ws, &challenge) != nil || challenge.Protocol != hostedProtocol ||
		challenge.DeploymentID != g.cfg.Image.DeploymentID || challenge.ConfigurationDigest != brokerJSONDigest(g.cfg.Image) ||
