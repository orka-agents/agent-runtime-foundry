...
func newHostedServer(ctx context.Context, cfg hostedImageConfig, getenv func(string) string, runner hostedSupervisorRunner) (*hostedServer, error) {
	...
	agent, err := readHostedFile(acpConfigPath, acpMaxConfigBytes)
	if err != nil || brokerSHA(agent) != cfg.AgentConfigurationDigest || !brokerDigestValid(digest) {
		return nil, errHostedInvalid
	}
	var nonce [32]byte
	...
	return &hostedServer{cfg: cfg, adapterDigest: digest, agentConfig: agent, runner: runner,
		ctx: lifetime, cancel: cancel, challenge: hostedChallenge{Protocol: hostedProtocol,
			DeploymentID: cfg.DeploymentID, ConfigurationDigest: brokerJSONDigest(cfg),
...
func (s *hostedServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if hello.Role == "forward" {
		...
		if err != nil || kind != websocket.TextMessage || brokerSHA(data) != pair.bootstrapDigest ||
			acpDecode(data, &bootstrap, true) != nil || validateHostedBootstrap(s.cfg, bootstrap) != nil ||
		...
		clear(data)
		if _, err := decodeACPAgentConfiguration(s.agentConfig, s.cfg.AgentConfigurationDigest, bootstrap.Environment["ORKA_ACP_MODEL"]); err != nil {
			pair.close()
			return
		}
		s.mu.Lock()
...
func hostedRoutingQuery(r *http.Request) bool {
	...
	for key, values := range query {
		if (key != "api-version" && key != "agent_session_id") || len(values) != 1 || len(values[0]) > 512 || !acpSafeString(values[0], 512) {
			return false
		}
	}
	return true
...
func readHostedWSJSON(ws *websocket.Conn, value any) error {
	kind, data, err := ws.ReadMessage()
	if err != nil || kind != websocket.TextMessage || len(data) > hostedMaxHandshakeBytes || acpDecode(data, value, true) != nil {
		return errHostedInvalid
	}
	return nil
}
