...
func hostedStartupGatewaySettings(t *testing.T) hostedGatewaySettings {
	...
	settings := hostedGatewaySettings{
		config: hostedGatewayConfig{
			...
			ContainerImage: "example.invalid/hosted@" + brokerSHA([]byte("startup fixture image")),
			SessionID:      f.hello.Challenge.SessionID, RuntimeProfileDigest: brokerSHA([]byte("startup fixture profile")),
...
func TestHostedStartupCreateReservationPrecedesSubmission(t *testing.T) {
	...
	f.gateway.httpClient.Transport = hostedGatewayTestRoundTripper(func(request *http.Request) (*http.Response, error) {
		if request.Method == http.MethodPost {
			...
			if err != nil || acpDecode(data, &ledger, true) != nil || !ledger.CreateAttempted || ledger.SessionCreated || ledger.ExposurePossible || ledger.PrincipalDigest == "" {
