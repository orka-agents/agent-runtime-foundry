...
type responsesBackend interface {
	CreateResponse(context.Context, foundry.ResponseRequest, foundry.ResponseCallbacks) (foundry.StreamSummary, error)
...
func (b foundryBackend) CreateResponse(ctx context.Context, request foundry.ResponseRequest, callbacks foundry.ResponseCallbacks) (foundry.StreamSummary, error) {
...
func (a *adapter) startTurn(request harness.StartTurnRequest) (*turnState, string, error) {
	...
	go a.runResponse(turn, foundry.ResponseRequest{
...
func (a *adapter) runResponse(turn *turnState, request foundry.ResponseRequest) {
	...
	recordInvocationIDs := func(responseID, sessionID string) error {
		if err := foundry.ValidateIdentifier("response id", responseID); err != nil {
			return err
		}
	...
	summary, err := a.backend.CreateResponse(turn.ctx, request, foundry.ResponseCallbacks{
		OnCreated: func(response foundry.Response) error {
		...
		OnFunctionCall: func(call foundry.OutputItem) error {
...
func (a *adapter) recordFunctionCall(turn *turnState, call foundry.OutputItem) error {
	providerCallID := call.CallID
	name := call.Name
	if err := foundry.ValidateCallID(providerCallID); err != nil {
		return err
	}
	if err := foundry.ValidateFunctionName(name); err != nil {
		return err
	}
...
func (a *adapter) prepareContinuationLocked(turn *turnState) (foundry.ResponseRequest, error) {
	if turn.continuationInFlight {
		return foundry.ResponseRequest{}, errors.New("foundry continuation is already in flight")
	}
	...
	if err := a.ensureNonterminalFrameCapacityLocked(turn, len(callIDs)); err != nil {
		return foundry.ResponseRequest{}, err
	}
	outputs := make([]foundry.FunctionOutput, 0, len(callIDs))
	for _, callID := range callIDs {
		...
		outputs = append(outputs, foundry.FunctionOutput{
	...
	previousResponseID := foundry.FirstNonBlank(turn.responseID, turn.previousResponse)
	if previousResponseID == "" {
		return foundry.ResponseRequest{}, errors.New("foundry continuation is missing previous_response_id")
	}
	for _, callID := range callIDs {
	...
	return foundry.ResponseRequest{
		Input:              outputs,
...
func validateAdapterStartRequest(request harness.StartTurnRequest) error {
	...
	if len(request.Input.Prompt) > foundry.MaxPromptBytes {
		return errors.New("prompt exceeds adapter limit")
	}
	if len(request.Input.Tools) > foundry.DefaultMaxBrokeredCalls {
	...
	if toolBytes > foundry.MaxToolSchemaBytes {
...
func validateFoundryToolDefinitions(definitions []harness.ToolDefinition) error {
	...
	for _, definition := range definitions {
		...
		if foundry.ValidateFunctionName(name) != nil {
...
func foundryToolSchemas(request harness.StartTurnRequest) []foundry.ToolSchema {
	if request.ToolExecutionMode != harness.ToolExecutionModeBrokered {
		return nil
	}
	tools := make([]foundry.ToolSchema, 0, len(request.Input.Tools))
	for _, definition := range request.Input.Tools {
		...
		tools = append(tools, foundry.ToolSchema{
			Type:        "function",
			Name:        strings.TrimSpace(definition.Name),
...
func (a *adapter) providerToolSchemas(request harness.StartTurnRequest) []foundry.ToolSchema {
	if strings.EqualFold(strings.TrimSpace(a.cfg.toolSchemaMode), foundry.ToolSchemaModeProviderStatic) {
		return nil
	}
...
func validateFoundryStreamSummary(summary foundry.StreamSummary) error {
	if err := foundry.ValidateIdentifier("response id", summary.ResponseID); err != nil {
		return err
	}
	return foundry.ValidateIdentifier("agent session id", summary.AgentSessionID)
}
...
func providerFailureMessage(providerError *foundry.ResponseError) string {
	if providerError == nil {
		return "Foundry response failed"
	}
	code := foundry.FirstNonBlank(providerError.Code, providerError.Type)
	if code == "" {
		return "Foundry response failed"
	}
...
func incompleteFailureMessage(incomplete *foundry.Incomplete) string {
...
func retryableResponseFailure(providerError *foundry.ResponseError) bool {
	if providerError == nil {
		return false
	}
	code := strings.ToLower(foundry.FirstNonBlank(providerError.Code, providerError.Type))
...
func providerSessionCheckpointInvalid(err error, request foundry.ResponseRequest) bool {
