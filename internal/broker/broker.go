...
func brokerRecordOperation(session *brokerSession, path string, c brokerContext) (bool, error) {
	...
	switch path {
	case brokerapi.SettlePath:
		limit = brokerSettlementOperationLimit
	case brokerapi.RetirePath:
...
func (b *lifecycleBroker) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	validPath := r.URL.Path == brokerapi.ResponsesPath || r.URL.Path == brokerapi.RenewPath || r.URL.Path == brokerapi.SettlePath ||
		r.URL.Path == brokerapi.RetirePath || r.URL.Path == brokerapi.StatusPath
	if !validPath || r.URL.RawQuery != "" || r.URL.RawPath != "" || r.Header.Get("Content-Encoding") != "" ||
		(r.URL.Path == brokerapi.StatusPath && r.Method != http.MethodGet) || (r.URL.Path != brokerapi.StatusPath && r.Method != http.MethodPost) {
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	body, err := io.ReadAll(io.LimitReader(r.Body, foundry.MaxPromptBytes+1))
	if err != nil || len(body) > foundry.MaxPromptBytes {
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	if (r.URL.Path == brokerapi.StatusPath && len(body) != 0) ||
		(r.URL.Path != brokerapi.ResponsesPath && r.URL.Path != brokerapi.StatusPath && string(body) != "{}") {
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if r.URL.Path == brokerapi.ResponsesPath {
	...
	if r.URL.Path == brokerapi.SettlePath || r.URL.Path == brokerapi.RetirePath {
		b.startReconcile(foundry.JSONDigest(c.Owner), true)
	}
	b.mu.Lock()
	response := b.controlResponseLocked(c, contextDigest)
	if r.URL.Path == brokerapi.RenewPath && b.canAcknowledgeCreateRenewalLocked(c, response) {
		response.State = "open"
	}
...
func (b *lifecycleBroker) control(path string, c brokerContext) error {
	...
	ordinary := path == brokerapi.RenewPath || session == nil ||
		(path == brokerapi.SettlePath && session.Prompts[c.promptKey()] == nil)
	...
	if err == nil && (path == brokerapi.SettlePath || path == brokerapi.RetirePath) {
		if active, ok := b.active[foundry.JSONDigest(c.Owner)]; ok && (path == brokerapi.RetirePath || active.prompt == c.promptKey()) {
			active.cancel()
		}
	}
	return err
...
func (b *lifecycleBroker) controlResponseLocked(c brokerContext, contextDigest string) brokerControlResponse {
	response := brokerControlResponse{Protocol: brokerProtocol, OwnerDigest: foundry.JSONDigest(c.Owner), OperationID: c.OperationID,
...
func (b *lifecycleBroker) reconcile(ctx context.Context, key string) {
	...
	_ = b.commitLocked(func(next *brokerLedger) error {
		...
		for _, promptKey := range closing {
			prompt := session.Prompts[promptKey]
			prompt.Settled = true
			prompt.ProofDigest = foundry.JSONDigest(struct {
				Owner, Prompt, Remote, Kind, At string
			}{key, promptKey, foundry.Digest([]byte(remoteID)), kind, time.Now().UTC().Format(time.RFC3339Nano)})
			for _, invocation := range prompt.Invocations {
		...
		session.ProofDigest = foundry.JSONDigest(struct{ Owner, Remote, Kind, At string }{
			key, foundry.Digest([]byte(remoteID)), kind, time.Now().UTC().Format(time.RFC3339Nano)})
		return nil
	})
}
