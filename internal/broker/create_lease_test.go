...
func TestBrokerCreationRequiresCurrentLeaseBeforeIntent(t *testing.T) {
	for _, renewed := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if strictjson.Decode(body, &request, true) != nil {
			...
			err = b.commitLocked(func(next *brokerLedger) error {
				...
				if _, ensureErr = brokerRecordOperation(session, brokerapi.ResponsesPath, c); ensureErr != nil {
					return ensureErr
				}
			...
			b.tokenProvider = brokerEvidenceTokenProvider(func(context.Context) (string, error) {
				if tokens.Add(1) == 3 && renewed {
					b.mu.Lock()
					err := b.commitLocked(func(next *brokerLedger) error {
						prompt := next.Sessions[foundry.JSONDigest(c.Owner)].Prompts[c.promptKey()]
			...
			_, err = b.invoke(ctx, c, request.ResponseRequest)
