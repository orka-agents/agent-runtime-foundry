...
func TestBrokerFoldedSessionEvidenceCannotSettleActiveOwner(t *testing.T) {
	...
	go func() {
		_, _, _ = brokerTestHTTP(context.Background(), server.URL, brokerapi.ResponsesPath, c, brokerTestBody(""))
		close(finished)
	}()
	...
	control, body := brokerTestControlContext(brokerapi.SettlePath, c)
	status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerapi.SettlePath, control, body)
