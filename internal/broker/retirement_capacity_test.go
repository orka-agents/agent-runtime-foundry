...
func brokerFillHistoricalOperationCapacity(t *testing.T, b *lifecycleBroker, c brokerContext, count int) (brokerContext, map[string]string) {
	...
	err := b.commitLocked(func(next *brokerLedger) error {
		...
		for len(session.Operations) < count {
			...
			session.Operations[renewal.OperationID] = brokerOperationDigest(brokerapi.RenewPath, renewal)
			prompt.LeaseGeneration = renewal.LeaseGeneration
		}
		return nil
	})
	valid := brokerLedgerValid(b.ledger, b.cfg.configDigest)
	operations := make(map[string]string, len(b.ledger.Sessions[foundry.JSONDigest(c.Owner)].Operations))
	for key, value := range b.ledger.Sessions[foundry.JSONDigest(c.Owner)].Operations {
...
func TestBrokerOperationCapacityPreservesSettlementAndRetirement(t *testing.T) {
	for _, settleFirst := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerapi.RenewPath, fresh, []byte("{}"))
			if err != nil || status != http.StatusServiceUnavailable {
				t.Fatal("saturated owner admitted more ordinary operation records")
			}
			status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerapi.RenewPath, renewal, []byte("{}"))
			if err != nil || status != http.StatusOK {
			...
			conflict, body := brokerTestControlContext(brokerapi.RetirePath, c)
			conflict.OperationID = renewal.OperationID
			status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerapi.RetirePath, conflict, body)
			if err != nil || status != http.StatusConflict {
			...
			if settleFirst {
				...
				extra, body := brokerTestControlContext(brokerapi.SettlePath, c)
				extra.OperationID = "extra-settlement-after-capacity"
				status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerapi.SettlePath, extra, body)
				if err != nil || status != http.StatusServiceUnavailable {
					t.Fatal("extra settlement consumed retirement capacity")
				}
			}
			proof := brokerTestControl(t, server.URL, brokerapi.RetirePath, c)
			if !proof.RetirementProven || proof.OwnerDigest != foundry.JSONDigest(c.Owner) || !foundry.DigestValid(proof.ProofDigest) {
			...
			for i := range 3 {
				extra, body := brokerTestControlContext(brokerapi.RetirePath, c)
				extra.OperationID = fmt.Sprintf("extra-retirement-%d", i)
				status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerapi.RetirePath, extra, body)
			...
			reopened := brokerTestControl(t, server.URL, brokerapi.RetirePath, c)
...
func TestBrokerOperationCapacityRetainsUnknownCreation(t *testing.T) {
	...
	retirement, body := brokerTestControlContext(brokerapi.RetirePath, c)
	status, data, err := brokerTestHTTP(context.Background(), server.URL, brokerapi.RetirePath, retirement, body)
...
func TestBrokerOperationCapacityCancelsAcknowledgedInvocation(t *testing.T) {
	...
	settlement, body := brokerTestControlContext(brokerapi.SettlePath, c)
	status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerapi.SettlePath, settlement, body)
...
func TestBrokerOperationCapacityRejectsOversizedLedger(t *testing.T) {
	cfg := brokerConfiguration{configDigest: foundry.Digest([]byte("bounded-capacity-fixture")), stateDir: filepath.Join(t.TempDir(), "broker")}
	c := brokerTestContext(cfg)
	...
	ledger.Sessions[foundry.JSONDigest(c.Owner)] = session
	for i := range brokerHistoricalOperationCapacity + 2 {
		session.Operations[fmt.Sprintf("historical-operation-%d", i)] = foundry.Digest([]byte(fmt.Sprintf("operation-%d", i)))
	}
	...
	ledger.Sessions[foundry.JSONDigest(c.Owner)].Operations["over-capacity"] = foundry.Digest([]byte("extra-operation"))
	err = store.save(ledger)
	store.close()
	if err != nil {
