...
func TestBrokerStorageFailureCancelsActiveRequests(t *testing.T) {
	for _, trigger := range []string{"expiry", "settle", "other-owner-renewal"} {
		t.Run(trigger, func(t *testing.T) {
			...
			if trigger != "expiry" {
				path, closing := brokerapi.SettlePath, c
				if trigger == "other-owner-renewal" {
					path = brokerapi.RenewPath
			...
			closing, body := brokerTestControlContext(brokerapi.SettlePath, c)
			status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerapi.SettlePath, closing, body)
			if err != nil || status != http.StatusServiceUnavailable {
...
func TestBrokerRetirementRejectsNewSettlementPrompt(t *testing.T) {
	for _, state := range []string{"retiring", "retired"} {
		t.Run(state, func(t *testing.T) {
			...
			if state == "retiring" {
				retire, body := brokerTestControlContext(brokerapi.RetirePath, c)
				status, _, err := brokerTestHTTP(context.Background(), server.URL, brokerapi.RetirePath, retire, body)
			...
			later, body := brokerTestControlContext(brokerapi.SettlePath, later)
			status, _, err = brokerTestHTTP(context.Background(), server.URL, brokerapi.SettlePath, later, body)
			...
			prompts := len(b.ledger.Sessions[foundry.JSONDigest(c.Owner)].Prompts)
			b.mu.Unlock()
			if !valid || prompts != 1 {
