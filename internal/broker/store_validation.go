...
func brokerLedgerValid(ledger *brokerLedger, digest string) bool {
	if ledger.Version != 1 || ledger.ConfigDigest != digest || ledger.Sessions == nil || len(ledger.Sessions) > 4096 ||
		(ledger.PrincipalDigest != "" && !foundry.DigestValid(ledger.PrincipalDigest)) {
		return false
	}
...
func brokerSessionValid(session *brokerSession, key, digest string) bool {
	if session == nil || !session.Owner.valid() || key != foundry.JSONDigest(session.Owner) ||
	...
	if session.Retired {
		if !session.Retiring || (session.CreateState != "none" && session.CreateState != "deleted") || !foundry.DigestValid(session.ProofDigest) {
			return false
		}
	...
	for id, operation := range session.Operations {
		if !foundry.SafeString(id, 512) || !foundry.DigestValid(operation) {
			return false
		}
	}
...
func brokerPromptValid(prompt *brokerPrompt, key, digest string, session *brokerSession, linked map[string]bool) bool {
	if prompt == nil || prompt.Identity.promptKey() != key || prompt.Identity.Owner != session.Owner ||
		...
		!foundry.SafeString(prompt.Identity.TaskUID, 512) || prompt.Identity.TaskAttempt == 0 ||
		!foundry.SafeString(prompt.Identity.PromptID, 512) || !foundry.DigestValid(prompt.Identity.PromptRequestDigest) ||
		!foundry.DigestValid(prompt.Identity.BodySHA256) || session.Operations[prompt.Identity.OperationID] == "" ||
	...
	if prompt.Settled {
		if !prompt.Closing || !foundry.DigestValid(prompt.ProofDigest) {
			return false
		}
...
func brokerInvocationValid(invocation *brokerInvocation, sequence uint64, promptKey string, prompt *brokerPrompt, session *brokerSession, linked map[string]bool) bool {
	if invocation == nil || invocation.Sequence != sequence || sequence == 0 || !foundry.DigestValid(invocation.BodyDigest) ||
		!foundry.SafeString(invocation.OperationID, 512) || session.Operations[invocation.OperationID] == "" {
		return false
	}
	...
	if !ok || linked[invocation.ResponseAlias] || !brokerAliasValid(invocation.ResponseAlias, "fr_") ||
		foundry.ValidateIdentifier("response", link.RemoteID) != nil || link.RemoteID != invocation.ResponseID ||
	...
	for alias, remote := range link.CallIDs {
		if !brokerAliasValid(alias, "fc_") || foundry.ValidateIdentifier("call", remote) != nil {
			return false
		}
	}
