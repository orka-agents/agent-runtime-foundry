...
func NewClient(baseURL string, opts ...ClientOption) (*Client, error) {
	...
	c.httpClient = harnessHTTPClient(c.httpClient)
	return c, nil
}

func harnessHTTPClient(source *http.Client) *http.Client {
	if source == nil {
		source = &http.Client{}
	}
	clone := *source
...
func (c *Client) Health(ctx context.Context) (*HealthResponse, error) {
	...
	if err := response.Validate(); err != nil {
		return nil, c.safeError("health", 0, err.Error())
	}
	return &response, nil
}
...
func (c *Client) Capabilities(ctx context.Context) (*CapabilitiesResponse, error) {
	...
	if err := response.Validate(); err != nil {
		return nil, c.safeError("capabilities", 0, err.Error())
	}
	return &response, nil
}
...
func (c *Client) StartTurn(ctx context.Context, request StartTurnRequest) (*StartTurnResponse, error) {
	...
	if err := c.postJSON(ctx, TurnsPath, request, &response); err != nil {
		...
		if errors.As(err, &clientErr) && clientErr.StatusCode >= http.StatusOK &&
			clientErr.StatusCode < http.StatusMultipleChoices {
			clientErr.RemoteAccepted = true
			return nil, clientErr
		}
		return nil, err
	}
	if strings.TrimSpace(response.Version) != ProtocolVersion {
		return nil, c.acceptedError("start_turn", fmt.Sprintf("unsupported version %q", response.Version))
	}
	if !response.Accepted {
		return nil, c.safeError("start_turn", 0, "harness did not accept turn")
	}
	if err := validateAcceptedTurn(request, response); err != nil {
		return nil, c.acceptedError("start_turn", err.Error())
	}
	return &response, nil
}
...
func (c *Client) CancelTurn(ctx context.Context, request CancelTurnRequest) (*CancelTurnResponse, error) {
	if err := request.Validate(); err != nil {
		return nil, c.safeError("cancel_turn", 0, err.Error())
	}
	...
	if strings.TrimSpace(response.Version) != ProtocolVersion {
		return nil, c.safeError("cancel_turn", 0, fmt.Sprintf("unsupported version %q", response.Version))
	}
	if !response.Accepted {
		return nil, c.safeError("cancel_turn", 0, "harness did not accept cancellation")
	}
	if response.RuntimeSessionID != request.RuntimeSessionID {
		return nil, c.safeError(
			"cancel_turn",
			0,
	...
	if response.TurnID != request.TurnID {
		return nil, c.safeError("cancel_turn", 0, fmt.Sprintf("harness cancelled turn %q, want %q", response.TurnID, request.TurnID))
	}
...
func (c *Client) ContinueTurn(ctx context.Context, request ContinueTurnRequest) (*ContinueTurnResponse, error) {
	if err := request.Validate(); err != nil {
		return nil, c.safeError("continue_turn", 0, err.Error())
	}
...
func (c *Client) FetchTurnOutput(ctx context.Context, turnID HarnessTurnID, outputRef string) ([]byte, error) {
	...
	if err != nil {
		return nil, c.safeError("fetch_turn_output", 0, safeTransportErrorMessage(err))
	}
...
func (c *Client) StreamFrames(ctx context.Context, turnID HarnessTurnID, afterSeq int64, emit func(HarnessEventFrame) error) error {
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return c.safeError("stream_frames", 0, safeTransportErrorMessage(err))
	}
	defer resp.Body.Close() //nolint:errcheck
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return c.statusError("stream_frames", resp)
	}
	return c.readSSEFrames(resp.Body, emit)
}
...
func (c *Client) getJSON(ctx context.Context, rel string, out any) error {
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return c.safeError("get", 0, safeTransportErrorMessage(err))
	}
	defer resp.Body.Close() //nolint:errcheck
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return c.statusError("get", resp)
	}
	return c.decodeBoundedJSON("get", resp.StatusCode, resp.Body, out)
}
...
func (c *Client) postJSON(ctx context.Context, rel string, in, out any) error {
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return c.safeError("post", 0, safeTransportErrorMessage(err))
	}
	defer resp.Body.Close() //nolint:errcheck
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return c.statusError("post", resp)
	}
	return c.decodeBoundedJSON("post", resp.StatusCode, resp.Body, out)
}

func (c *Client) decodeBoundedJSON(op string, status int, body io.Reader, out any) error {
	data, err := io.ReadAll(io.LimitReader(body, maxHarnessControlResponseBytes+1))
	if err != nil {
		return c.safeError(op, status, err.Error())
	}
	if len(data) > maxHarnessControlResponseBytes {
		return c.safeError(op, status, "JSON response exceeds harness control limit")
	}
	if out == nil {
		return nil
	}
...
func (c *Client) statusError(op string, resp *http.Response) error {
	...
	return c.safeError(op, resp.StatusCode, resp.Status)
}
...
func (c *Client) readSSEFrames(r io.Reader, emit func(HarnessEventFrame) error) error {
	...
	for scanner.Scan() {
		line := scanner.Text()
		if line == "" {
			if err := c.emitSSEData(data.String(), emit); err != nil {
	...
	if err := scanner.Err(); err != nil {
		return c.safeError("stream_frames", 0, err.Error())
	}
	if data.Len() > 0 {
		if err := c.emitSSEData(data.String(), emit); err != nil {
			if errors.Is(err, errSSEDone) {
...
func (c *Client) emitSSEData(raw string, emit func(HarnessEventFrame) error) error {
	...
	if err := json.Unmarshal([]byte(raw), &frame); err != nil {
		return c.safeError("stream_frames", 0, fmt.Sprintf("decode harness frame: %v", err))
	}
...
func (c *Client) sanitizeMessage(message string) string {
	if c != nil && c.authBearerValue != "" {
		message = strings.ReplaceAll(message, c.authBearerValue, "[REDACTED]")
	}
	return events.RedactExecutionEventText(message)
}

func (c *Client) acceptedError(op, message string) error {
	return ClientError{
		Op:             op,
...
func (c *Client) safeError(op string, status int, message string) error {
	return ClientError{Op: op, StatusCode: status, Message: c.sanitizeMessage(message)}
}
