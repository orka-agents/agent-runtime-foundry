...
func hostedBoundaryLedgerFixture(t *testing.T) (hostedGatewayConfig, hostedGatewayLedger) {
	...
	cfg := hostedGatewayConfig{
		...
		ContainerImage: "example.invalid/hosted@" + foundry.Digest([]byte("fixture image")),
		SessionID:      f.hello.Challenge.SessionID, RuntimeProfileDigest: foundry.Digest([]byte("fixture profile")),
	...
	ledger := hostedGatewayLedger{
		Version: 1, ConfigDigest: foundry.JSONDigest(cfg), SessionID: cfg.SessionID,
		PrincipalDigest: foundry.Digest([]byte("fixture principal")), CreateAttempted: true, SessionCreated: true,
...
func TestHostedBoundaryLedgerRejectsMixedAndMalformedExposure(t *testing.T) {
	...
	for name, mutate := range map[string]func(*hostedGatewayLedger){
		...
		"config digest":        func(v *hostedGatewayLedger) { v.ConfigDigest = foundry.Digest([]byte("other config")) },
		...
		"challenge config": func(v *hostedGatewayLedger) {
			v.Challenge.ConfigurationDigest = foundry.Digest([]byte("other image config"))
		},
...
func TestHostedBoundaryLedgerRejectsMalformedEncodingAndConfigChanges(t *testing.T) {
	...
	for name, mutate := range map[string]func(*hostedGatewayConfig){
		...
		"profile": func(c *hostedGatewayConfig) { c.RuntimeProfileDigest = foundry.Digest([]byte("other profile")) },
		"image": func(c *hostedGatewayConfig) {
			c.ContainerImage = "example.invalid/hosted@" + foundry.Digest([]byte("other image"))
		},
...
func TestHostedBoundaryLedgerPersistsExposureWithExclusivePrivateOwnership(t *testing.T) {
	...
	for _, name := range []string{"state.json", "gateway.lock"} {
		...
		if err != nil || !durablestore.PrivateFile(info) {
...
func TestHostedBoundaryProxyRoutes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"broker responses", http.MethodPost, brokerapi.ResponsesPath, hostedBrokerRoute, true},
		{"broker renew", http.MethodPost, brokerapi.RenewPath, hostedBrokerRoute, true},
		{"broker settle", http.MethodPost, brokerapi.SettlePath, hostedBrokerRoute, true},
		{"broker retire", http.MethodPost, brokerapi.RetirePath, hostedBrokerRoute, true},
		{"broker status GET", http.MethodGet, brokerapi.StatusPath, hostedBrokerRoute, true},
		{"broker status POST denied", http.MethodPost, brokerapi.StatusPath, hostedBrokerRoute, false},
		{"broker response GET denied", http.MethodGet, brokerapi.ResponsesPath, hostedBrokerRoute, false},
		{"broker query denied", http.MethodPost, brokerapi.ResponsesPath + "?target=other", hostedBrokerRoute, false},
...
func TestHostedBoundaryProxyPinsDestinationAndPreservesOnlySuppliedAuthorization(t *testing.T) {
	for _, test := range []struct {
		...
		{"broker", brokerapi.RenewPath, hostedBrokerRoute},
