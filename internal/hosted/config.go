...
func readHostedConfig(path string, target any) error {
	...
	if err != nil || strictjson.Decode(data, target, true) != nil {
...
func loadHostedGatewaySettings(path string, getenv func(string) string) (hostedGatewaySettings, error) {
	...
	settings.address = foundry.FirstNonBlank(getenv("ORKA_FOUNDRY_GATEWAY_ADDR"), ":8080")
...
func validateHostedGatewayConfig(cfg hostedGatewayConfig) error {
	...
	if cfg.Protocol != hostedProtocol || validateHostedImageConfig(cfg.Image) != nil ||
		!hostedUUIDValid(cfg.SessionID) || !foundry.DigestValid(cfg.RuntimeProfileDigest) ||
		!pinned || !foundry.DigestValid(digest) || !foundry.SafeString(imageName, 512) ||
...
func hostedRelayTargetValid(raw string, loopback bool) bool {
	u, err := url.Parse(raw)
	if err != nil || !foundry.SafeString(raw, 2048) || (u.Scheme != "http" && u.Scheme != "https") ||
