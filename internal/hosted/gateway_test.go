...
func newHostedGatewayTestFixture(t *testing.T, options hostedGatewayTestOptions) *hostedGatewayTestFixture {
	...
	f.settings = hostedGatewaySettings{
		config: hostedGatewayConfig{
			...
			ContainerImage: "example.invalid/hosted@" + foundry.Digest([]byte("gateway fixture image")),
			SessionID:      p.hello.Challenge.SessionID, RuntimeProfileDigest: foundry.Digest([]byte("gateway fixture profile")),
...
func (f *hostedGatewayTestFixture) serveHTTP(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path == brokerapi.StatusPath || r.URL.Path == "/internal/v2/acp/mcp/tools/call" {
		f.callbacks.Add(1)
	...
	case r.Method == http.MethodPost && r.URL.Path == base+"/endpoint/sessions":
		...
		if err != nil || strictjson.Decode(data, &request, true) != nil ||
...
func (f *hostedGatewayTestFixture) serveWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	challenge := hostedChallenge{
		...
		ConfigurationDigest: foundry.JSONDigest(f.settings.config.Image),
	...
	valid := kind == websocket.TextMessage && foundry.Digest(data) == hello.BootstrapDigest &&
		strictjson.Decode(data, &bootstrap, true) == nil && reflect.DeepEqual(bootstrap, f.settings.bootstrap)
	clear(data)
	if !valid {
...
func (f *hostedGatewayTestFixture) observeBootstrapWrite() {
	...
	observed.persisted = err == nil && strictjson.Decode(data, &observed.ledger, true) == nil &&
...
func TestHostedGatewayBindsLifetimeAndPersistsBeforeBootstrap(t *testing.T) {
	...
	if !observed.persisted || observed.containsToken || !observed.ledger.ExposurePossible ||
		...
		observed.ledger.PrincipalDigest == "" || observed.ledger.BootstrapDigest != foundry.JSONDigest(f.settings.bootstrap) {
	...
	want := []string{"GET " + base + "?api-version=v1",
		...
		"GET " + base + foundry.SessionSuffix(f.settings.config.SessionID) + "?api-version=v1",
	...
	for _, endpoint := range []struct{ method, url string }{
		{http.MethodGet, "http://broker" + brokerapi.StatusPath},
...
func TestHostedGatewayRejectsRemoteTargetDriftBeforeChannels(t *testing.T) {
	for _, test := range []struct {
		...
		{"container digest", "version", func(v map[string]any) {
			v["definition"].(map[string]any)["container_configuration"] = map[string]any{"image": "example.invalid/hosted@" + foundry.Digest([]byte("other image"))}
		}},
...
func TestHostedGatewayRejectsChallengeAndChannelBindingDrift(t *testing.T) {
	for name, mutate := range map[string]func(*hostedChallenge){
		...
		"configuration": func(c *hostedChallenge) { c.ConfigurationDigest = foundry.Digest([]byte("other configuration")) },
	...
	for name, mutate := range map[string]func(*hostedAccepted){
		...
		"bootstrap": func(a *hostedAccepted) { a.BootstrapDigest = foundry.Digest([]byte("other bootstrap")) },
	} {
...
func TestHostedGatewayRejectsReadyAndCapabilityDriftAfterExposure(t *testing.T) {
	...
	for name, mutate := range map[string]func(map[string]any){
		...
		"profile":   func(v map[string]any) { v["runtimeProfileDigest"] = foundry.Digest([]byte("other profile")) },
		"adapter": func(v map[string]any) {
			v["adapterDigests"] = map[string]string{"foundry-serve-acp": foundry.Digest([]byte("other adapter"))}
		},
	} {
