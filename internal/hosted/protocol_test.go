...
func newHostedProtocolFixture(t *testing.T) hostedProtocolFixture {
	...
	cfg := hostedImageConfig{
		...
		AgentConfigurationDigest: foundry.Digest([]byte("test agent configuration")),
	}
	bootstrap := hostedBootstrap{
		...
		Environment: map[string]string{
			...
			"ORKA_ACP_FOUNDRY_ADAPTER_DIGEST":     foundry.Digest([]byte("test adapter")),
			"ORKA_ACP_WORKSPACE_INTENT":           "read",
			"ORKA_ACP_AGENT_CONFIGURATION_DIGEST": cfg.AgentConfigurationDigest,
			"ORKA_ACP_TOOL_POLICY_DIGEST":         foundry.Digest([]byte("test tool policy")),
			"ORKA_ACP_APPROVAL_POLICY_DIGEST":     foundry.Digest([]byte("test approval policy")),
			"ORKA_ACP_MCP_CONFIGURATION_DIGEST":   foundry.Digest([]byte("test MCP configuration")),
	...
	hello := hostedHello{
		Challenge: hostedChallenge{
			...
			ConfigurationDigest: foundry.JSONDigest(cfg), AgentName: cfg.Target.AgentName,
		...
		PairID: uuid.NewString(), Role: "forward", BootstrapDigest: foundry.JSONDigest(bootstrap),
...
func TestHostedHelloBindsEverySignedField(t *testing.T) {
	...
	for name, mutate := range map[string]func(*hostedHello){
		...
		"configuration": func(v *hostedHello) { v.Challenge.ConfigurationDigest = foundry.Digest([]byte("other configuration")) },
		...
		"bootstrap body": func(v *hostedHello) {
			...
			v.BootstrapDigest = foundry.JSONDigest(body)
		},
...
func TestHostedHelloCanonicalJSONWire(t *testing.T) {
	...
	if strictjson.Decode(raw, &reordered, true) != nil {
	...
	if strictjson.Decode(raw, &decoded, true) != nil ||
...
func TestHostedBootstrapRejectsInvalidValues(t *testing.T) {
	...
	for key, values := range map[string][]string{
		...
		"ORKA_ACP_AGENT_CONFIGURATION_DIGEST": {"", foundry.Digest([]byte("other agent configuration"))},
