...
func newHostedServerTestFixture(t *testing.T, runnerOK bool) *hostedServerTestFixture {
	...
	agent, err := json.Marshal(foundry.AgentConfig{Model: "test-model", ToolSchemaMode: foundry.ToolSchemaModeRequest,
		HostedTarget: f.protocol.config.Target})
	if err != nil {
		t.Fatal("could not prepare hosted agent fixture")
	}
	f.protocol.config.AgentConfigurationDigest = foundry.Digest(agent)
	f.protocol.bootstrap.Environment["ORKA_ACP_AGENT_CONFIGURATION_DIGEST"] = foundry.Digest(agent)
	f.protocol.hello.Challenge.ConfigurationDigest = foundry.JSONDigest(f.protocol.config)
	f.protocol.hello.BootstrapDigest = foundry.JSONDigest(f.protocol.bootstrap)
	if validateHostedBootstrap(f.protocol.config, f.protocol.bootstrap) != nil {
		t.Fatal("invalid canonical bootstrap fixture")
	}
	if _, err := foundry.DecodeAgentConfig(agent, f.protocol.config.AgentConfigurationDigest, "test-model"); err != nil {
...
func (f *hostedServerTestFixture) sendBootstrap(t *testing.T, ws *websocket.Conn) {
	...
	if err != nil || foundry.Digest(data) != f.protocol.hello.BootstrapDigest || ws.WriteMessage(websocket.TextMessage, data) != nil {
...
func TestHostedServerUnauthenticatedInputCannotReservePair(t *testing.T) {
	for _, name := range []string{"missing signature", "invalid signature", "different boot", "expired", "binary hello", "bootstrap before hello", "malformed JSON"} {
		t.Run(name, func(t *testing.T) {
			...
			ws := f.connect(t)
			hello := f.hello(t, "forward", uuid.NewString(), foundry.JSONDigest(f.protocol.bootstrap))
	...
	for _, name := range []string{"forward replay", "different pair", "different bootstrap"} {
		t.Run(name, func(t *testing.T) {
			...
			case "different bootstrap":
				otherDigest = foundry.Digest([]byte("different bootstrap"))
			}
			ws := f.connect(t)
	...
	for _, name := range []string{"wrong digest", "binary bootstrap", "unknown member", "duplicate member", "missing controller token", "model mismatch", "adapter mismatch", "inherited identity", "supervisor override", "invalid agent config"} {
		t.Run(name, func(t *testing.T) {
			...
			case "adapter mismatch":
				bootstrap.Environment["ORKA_ACP_FOUNDRY_ADAPTER_DIGEST"] = foundry.Digest([]byte("other adapter"))
			...
			digest := foundry.Digest(data)
			if name == "wrong digest" {
				digest = foundry.Digest([]byte("other bytes"))
			}
...
func TestHostedServerRunningLifetimeCannotLaunchSecondSupervisor(t *testing.T) {
	...
	for _, role := range []string{"forward", "reverse"} {
		ws := f.connect(t)
		if ws.WriteJSON(f.hello(t, role, uuid.NewString(), foundry.JSONDigest(f.protocol.bootstrap))) != nil {
