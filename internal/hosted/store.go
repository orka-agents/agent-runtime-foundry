...
func openHostedGatewayStore(dir string, cfg hostedGatewayConfig) (*hostedGatewayStore, hostedGatewayLedger, error) {
	...
	lock, created, err := durablestore.OpenLock(dir, "gateway.lock", durablestore.SyncDirectory)
	if err != nil {
		return nil, empty, errHostedInvalid
	}
	store := &hostedGatewayStore{dir: dir, lock: lock}
	ledger := hostedGatewayLedger{Version: 1, ConfigDigest: foundry.JSONDigest(cfg), SessionID: cfg.SessionID}
	...
	if os.IsNotExist(err) {
		...
		if readErr == nil && strictjson.Decode(data, &stored, true) == nil && hostedGatewayLedgerValid(stored, cfg) {
...
func hostedGatewayLedgerValid(value hostedGatewayLedger, cfg hostedGatewayConfig) bool {
	if value.Version != 1 || value.ConfigDigest != foundry.JSONDigest(cfg) || value.SessionID != cfg.SessionID ||
		(value.PrincipalDigest != "" && !foundry.DigestValid(value.PrincipalDigest)) ||
	...
	return value.SessionCreated && value.PrincipalDigest != "" && hostedUUIDValid(value.PairID) &&
		...
		foundry.DigestValid(value.BootstrapDigest) && value.Challenge.SessionID == cfg.SessionID &&
		value.Challenge.ConfigurationDigest == foundry.JSONDigest(cfg.Image) &&
