...
var (
	...
	transactionHeaderRe        = regexp.MustCompile(`(?i)\b(txn-token\s*:\s*)[A-Za-z0-9._~+/=-]+`)
	...
	naturalLanguageSensitiveRe = regexp.MustCompile(`(?i)\b((?:api\s+key|token|secret|password|credential)\s+is\s+)([^\s,.;]+)`) // e.g. "token is abc123"
	wellKnownSensitiveRe       = regexp.MustCompile(`\b(?:sk-[A-Za-z0-9_-]{20,}|gh[pousr]_[A-Za-z0-9_]{20,}|github_pat_[A-Za-z0-9_]{30,}|xox[baprs]-[A-Za-z0-9-]{20,})\b`)
...
func SensitiveText(s string) string {
	...
	s = transactionHeaderRe.ReplaceAllString(s, `${1}`+redactedValue)
	s = sensitiveAssignmentRe.ReplaceAllString(s, `${1}${2}${3}${4}`+redactedValue)
	s = naturalLanguageSensitiveRe.ReplaceAllString(s, `${1}`+redactedValue)
	s = urlCredentialRe.ReplaceAllString(s, `${1}`+redactedValue+`@`)
	s = wellKnownSensitiveRe.ReplaceAllString(s, redactedValue)
