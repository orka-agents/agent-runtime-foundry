...
func openStoreLock(dir, name string, syncParent func(string) error) (*os.File, bool, error) {
	if err := makeStoreDirectory(dir, syncParent); err != nil {
	...
	if info, err := lock.Stat(); err != nil || !brokerPrivateFile(info) {
		_ = lock.Close()
		return nil, false, os.ErrPermission
	}
	if err := flockStoreFile(lock, created); err != nil {
		_ = lock.Close()
		return nil, false, err
	}
...
func flockStoreFile(lock *os.File, created bool) error {
...
func makeStoreDirectory(dir string, syncParent func(string) error) error {
	...
	if err := makeStoreDirectory(parent, syncParent); err != nil {
		return err
	}
