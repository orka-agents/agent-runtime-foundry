...
func TestStoreDirectoryRetriesFailedParentBarrier(t *testing.T) {
	for _, failure := range []string{"first", "second", "ledger"} {
		t.Run(failure, func(t *testing.T) {
			...
			failedCalls := 0
			for attempt := range 2 {
				lock, _, err := openStoreLock(dir, "retry.lock", func(parent string) error {
					if parent == failedParent {
						failedCalls++
						return syscall.EIO
					}
					return syncStoreDirectory(parent)
				})
				if lock != nil {
					_ = lock.Close()
			...
			retried := false
			lock, created, err := openStoreLock(dir, "retry.lock", func(parent string) error {
				if parent == failedParent {
					retried = true
				}
...
func TestStoreDirectoryCompetingMkdirRequiresParentBarrier(t *testing.T) {
	...
	lock, _, err := openStoreLock(dir, "retry.lock", func(path string) error {
		if path == root {
		...
		return syncStoreDirectory(path)
	})
	if lock != nil {
		_ = lock.Close()
	...
	lock, created, err := openStoreLock(dir, "retry.lock", syncStoreDirectory)
